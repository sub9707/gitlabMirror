...
const BattleRank = ({ searchInput, searchRequestSign }: { searchInput: string; searchRequestSign: boolean }) => {
      ...
      {!isLoading && (
        <div>
          <div className={`${styles['top-3-div']} hidden md:flex`}>
            {top3.map((data, index) => (
              <div key={index} className={styles[`top-${index + 1}`]} onClick={() => onClickRepoItem(data.repoId)}>
                ...
                <div className={styles['img-div']}>
                  <Image alt={`top${index + 1}`} src={`/static/models_png/${pretreatModelUrl(data.repomonUrl)}.png`} width={150} height={120}></Image>
                ...
                <div className={styles['info-div']}>
                  ...
                  <p className={styles['card-content']}>{data.rating.toLocaleString()}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {battleRankInfo?.content.map((item, index) => (
              <div key={index} className={styles['list-item']}>
                ...
                <span
                  ...
                  <Image alt='레포몬' src={`/static/models_png/${pretreatModelUrl(item.repomonUrl)}.png`} width={35} height={35}></Image>
                  {item.repomonNickname}
                </span>
                <span onClick={() => onClickRepoItem(item.repoId)} className={styles.hover}>
                  {item.repoName}
                </span>
                <span onClick={() => onClickUserItem(item.userId)} className={styles.hover}>
                  {item.repoOwner}
                </span>
                <span>
